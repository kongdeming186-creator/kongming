...
const COLORS = ['#c41e24', '#d4a843', '#e63c40', '#b8963f', '#a01820', '#f0c860', '#8b1014', '#e8b94a'];

type TabKey = 'citymgmt' | 'dept';

export default function RightPieTabs({ className }: { className?: string }) {
  const { data } = useWeeklyAnalysis();
  const [active, setActive] = useState<TabKey>('citymgmt');

  const cityMgmt = data.city_mgmt_pie;
  const dept = data.dept_pie;
        ...
        series: [
          {
            ...
            data: cityMgmt.map((s, i) => ({
              name: s.name,
              value: s.value,
              itemStyle: { color: i === 0 ? '#c41e24' : '#d4a843' },
            })),
          },
        ],
      };
    ...
    >
      <div className="flex h-full min-h-0 flex-col gap-2">
        ...
        <div className="flex shrink-0 gap-4 border-b border-border/60">
          {[
            { key: 'citymgmt' as TabKey, label: '大城管占比', icon: ShieldCheck },
            { key: 'dept' as TabKey, label: '科室划分', icon: Building2 },
          ].map((tab) => {
            const Icon = tab.icon;
